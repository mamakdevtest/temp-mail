...
export default function AutomationCenter({ token }) {
  ...
  const createKey = async () => {
    try {
      const response = await apiFetch('/api/automation/api-keys', { method: 'POST', headers: { ...headers, 'Content-Type': 'application/json' }, body: JSON.stringify({ name: keyName, scopes: keyScopes }) });
      const data = await response.json(); if (!response.ok) throw new Error(data.message || data.error);
      setSecret(data.secret); setKeyName(''); await load();
    } catch (error) { setMessage(error.message); }
  };
  ...
  return <section className="ops-page automation-page"><header className="ops-page-header"><div><p className="ops-eyebrow"><BellRing size={14} /> AUTOMATION CENTER</p><h1>İnbox’ını ürünlerine bağla.</h1><p>API anahtarları ve webhook’lar hesap bazlıdır; sırları yalnız bir kez gösteririz.</p></div></header>
    ...
    <div className="automation-grid"><article className="ops-card"><div className="ops-card-heading"><div><span className="ops-step">API</span><h2>API anahtarı</h2></div><KeyRound size={18} /></div><label className="ops-field"><span>Ad</span><input className="input" value={keyName} onChange={(event) => setKeyName(event.target.value)} placeholder="Local test runner" /></label><div className="ops-field"><span>İzinler</span><div className="choice-grid">{SCOPE_OPTIONS.map((scope) => <label key={scope}><input type="checkbox" checked={keyScopes.includes(scope)} onChange={() => toggle(keyScopes, setKeyScopes, scope)} />{scope}</label>)}</div></div><button className="btn-primary w-full" onClick={createKey}><Plus size={16} /> Anahtar oluştur</button><div className="automation-list">{keys.map((key) => <article key={key.id}><div><strong>{key.name}</strong><code>{key.key_prefix}••••</code><small>{key.scopes.join(', ')}</small></div><button className="icon-button" title="İptal et" onClick={() => revoke(key.id)}><Trash2 size={15} /></button></article>)}</div></article>
